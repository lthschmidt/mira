...
if config.save_path_dataset:
    model = Train_MIME(vocab, decoder_number=program_number, model_file_path=config.saved_model_path)
else:
...
try:
    ...
    for n_iter in tqdm(range(100000)):
        ...
        if ((n_iter + 1) % check_iter == 0):
            ...
            model.scheduler.step(loss_val)
            # if acc_val> 0.40:
            #     for param in model.emoji_embedding.parameters():
            #         param.requires_grad = False
            #     for param in model.s_weight.parameters():
            #         param.requires_grad = False
            ...
            if patient > 3:
                ...
                patient = 0  # Сбрасываем терпение и продолжаем
